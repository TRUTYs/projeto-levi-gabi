...
@estoque_bp.route("/confirmar_adicionar_produto/<int:pid>/<int:qtd>")
@login_required
def confirmar_adicionar_produto(pid, qtd):
    p = EstoqueService.buscar_produto(pid)
    if p:
        EstoqueService.adicionar_quantidade(p, qtd)
        flash(f'{qtd} unidade(s) adicionada(s) a "{p.nome}".', "sucesso")
    return redirect(url_for("estoque.estoque"))

@estoque_bp.route("/editar_produto/<int:pid>", methods=["POST"])
@login_required
...
@estoque_bp.route("/excluir_produto/<int:pid>")
@login_required
...
@estoque_bp.route("/vender_produto/<int:pid>", methods=["POST"])
@login_required
def vender_produto(pid):
    p = EstoqueService.buscar_produto(pid)
    if not p: flash("Produto não encontrado.", "erro"); return redirect(url_for("estoque.estoque"))
    try:
...
@estoque_bp.route("/comprar_produto/<int:pid>", methods=["POST"])
@login_required
def comprar_produto(pid):
    p = EstoqueService.buscar_produto(pid)
    if not p: flash("Produto não encontrado.", "erro"); return redirect(url_for("estoque.estoque"))
    try:
        qtd = int(request.form.get("quantidade",0))
